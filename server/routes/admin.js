...
import { areNearDuplicateQuestions, cleanQuestionText, normalizeQuestion, parseAnswerPattern } from '../parser.js';
...
router.get('/pending', requireAdmin, async (req, res) => {
  try {
    ...
    const questionsRes = await query(
      `SELECT id, question_text, options, times_seen, source, created_at 
       FROM questions 
    ...
    const items = questionsRes.rows.map(row => {
      let options = [];
      try {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const existingAnswered = exactAnswered.rows.length > 0
        ? exactAnswered
        : {
            rows: (await query("SELECT id, normalized_text FROM questions WHERE answer_status = 'answered'"))
    ...
    res.status(500).json({ success: false, error: 'Failed to import answered questions' });
  }
});
