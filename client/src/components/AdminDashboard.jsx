...
Formatting Rules:
- Wrap each question's answer in curly brackets: { ... }
...
- Output ONLY the bracketed answers in sequential order. Do not include question numbers or conversational text.`;
  ...
  const handleCopyPromptWithSelectedQuestions = () => {
    ...
    const formattedQuestions = selected.map((q, idx) => {
      const opts = (q.options || []).map((o) => `   ${o.key}. ${o.text}`).join('\n');
      return `Question ${idx + 1} (ID: #${q.id}):\n${q.question_text}\n${opts}`;
    }).join('\n\n');

    const promptText = `Please solve the following ${selected.length} questions and output the answers strictly in the following pattern:
{ a delhi } { b india } { a delhi , c canada }

Rules:
- Wrap each question's answer in curly braces: { ... }
- For single choice: { key option_text } e.g. { a delhi }
- For multiple choices: { key option_text , key option_text } e.g. { a delhi , c canada }
- Provide exactly ${selected.length} answer block(s) in order, one per question.
- No numbering or conversational filler.
