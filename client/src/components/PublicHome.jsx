...
export default function PublicHome({ onNavigateAdmin }) {
  ...
  // Top toggle state: 'search' | 'upload'
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      {/* TOP TOGGLE: SEARCH / UPLOAD */}
      <div className="flex flex-col items-center justify-center space-y-2">
        <div className="inline-flex p-1.5 bg-slate-900 border border-slate-700 rounded-2xl shadow-xl backdrop-blur-md">
          <button
            type="button"
          ...
          >
            <Upload className="w-4 h-4" /> Upload / Submit Questions
          </button>
        </div>
      </div>

      {/* MODE 1: SEARCH QUESTIONS */}
      {activeMode === 'search' && (
        <section className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-6 sm:p-8 shadow-xl backdrop-blur-sm animate-fade-in">
          ...
          <div className="mt-8 pt-4 border-t border-slate-700/60 text-center">
            ...
            >
              <Upload className="w-4 h-4" /> Want to submit new questions? Click to switch to Upload Mode →
            </button>
          </div>
        </section>
      )}

      {/* MODE 2: UPLOAD / SUBMIT QUESTIONS (IN THE TOP WHEN ACTIVE) */}
      {activeMode === 'upload' && (
      <section className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-6 sm:p-8 shadow-xl backdrop-blur-sm">
        <div className="border-b border-slate-700 pb-5 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white">SUBMIT QUESTIONS</h2>
        ...
        <div className="mt-8 pt-4 border-t border-slate-700/60 text-center">
          <button
            ...
            className="text-xs sm:text-sm text-blue-400 hover:text-blue-300 font-semibold inline-flex items-center gap-1.5 transition"
          >
            <Search className="w-4 h-4" /> Want to search verified answers? Click to switch to Search Mode →
          </button>
        </div>
      </section>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-6">
            ...
            {submissionResult.isSingle ? (
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-700 text-sm text-slate-200">
